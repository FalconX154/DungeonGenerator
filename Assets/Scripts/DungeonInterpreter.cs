...
[CreateAssetMenu(fileName = "Dungeon Interpreter", menuName = "Dungeon Assets")]

public sealed class DungeonInterpreter : ScriptableObject, ILayoutInterpreter
{
    [SerializeField] public DungeonRoom room1Door;
    [SerializeField] public DungeonRoom room2DoorC;
    [SerializeField] public DungeonRoom room2DoorS;
    [SerializeField] public DungeonRoom room3Door;
    [SerializeField] public DungeonRoom room4Door;
    ...
    private List<DungeonRoom> instantiatedRooms = new List<DungeonRoom>();
    ...
    public void InterpretLayout(ERoomType[,] _layout)
    {
        ...
        for (int x = 0; x < currentLayoutLength; x++)
        {
            for (int y = 0; y < currentLayoutWidth; y++)
            {
                ...
                DungeonRoom newRoom = Instantiate(prefabToSpawn);
    ...
    {
        for (int i = instantiatedRooms.Count - 1; i >= 0; i--)
        {
        ...
        instantiatedRooms = new List<DungeonRoom>();

        currentLayout = new ERoomType[currentLayoutLength, currentLayoutWidth];
        for (int x = 0; x < currentLayoutLength; x++)
        {
            for (int y = 0; y < currentLayoutWidth; y++)
            {
                currentLayout[x, y] = ERoomType.free;
            }
        }
    }
    ...
    private DungeonRoom SelectPrefab(int _neighbourCount, bool[] _hasRoomInDirection)
    {
        switch (_neighbourCount)
        {
            ...
            case 2:
                if ((_hasRoomInDirection[0] && _hasRoomInDirection[2]) || (_hasRoomInDirection[1] && _hasRoomInDirection[3]))
                {
                    return room2DoorS;
                }
                else
    ...
    private float DetermineRotation(int _neighbourCount, bool[] _hasRoomInDirection)
    {
        ...
        {
            case 1:
                if (_hasRoomInDirection[0]) return 90f;
                if (_hasRoomInDirection[1]) return 180f;
                if (_hasRoomInDirection[2]) return -90f;
                return 0f;

            case 2:
                if (_hasRoomInDirection[0] && _hasRoomInDirection[2]) return 90f;
                if (_hasRoomInDirection[1] && _hasRoomInDirection[3]) return 0f;
                if (_hasRoomInDirection[0] && _hasRoomInDirection[1]) return 90f;
                if (_hasRoomInDirection[0] && _hasRoomInDirection[3]) return 0f;
                if (_hasRoomInDirection[1] && _hasRoomInDirection[2]) return 180f;
                return -90f;

            case 3:
                if (!_hasRoomInDirection[0]) return -90f;
                if (!_hasRoomInDirection[1]) return 0f;
                if (!_hasRoomInDirection[2]) return 90f;
                return 180f;
